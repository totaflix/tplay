...
<main class="">
    <title>LOKIIPTV</title>
    <div class="container">
        <div class="flex flex-row mb-3 ml-1 gap-3 justify-center align-middle" style="margin: 25px;">
            <input type="search" id="searchInput" class="relative w-[100%] col-span-4 block mx-4 rounded-l border border-solid border-neutral-300 bg-transparent bg-clip-padding px-3 py-1.5 text-base font-normal text-neutral-700 outline-none transition duration-300 ease-in-out focus:border-primary focus:text-neutral-700 focus:shadow-te-primary focus:outline-none dark:text-neutral-600 dark:placeholder:text-neutral-700" placeholder="Search">
        </div>
        <div id="listContainer" class="container grid place-items-center grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 mx-auto">
            ...
            <?php
            $id = $_GET['id'] ?? null; // Get the 'id' parameter from the URL (optional)

            $jsonString = file_get_contents('https://ayna-api.iptvbd.xyz/api/aynaott.json');
            $data = json_decode($jsonString, true);

            if ($data === null || !isset($data['channels'])) {
                echo "<p>Error decoding JSON or channels not found.</p>";
                exit;
            }

            $channelsData = $data['channels']; // Fetch the 'channels' array

            $category = ''; // Define the $category variable
            ?>

            <?php foreach ($channelsData as $channel): ?>
                <?php if ($category === '' || $category === $channel['categoryName']): ?>
                    <div class="box1">
                        <a href="play-ayna.php?id=<?php echo $channel['id']; ?>" data-sveltekit-preload-data="hover">
                            <div class="card w-40 bg-gray-100 rounded-xl bg-base-100 shadow-lg h-48 mb-4 flex flex-col">
                                <img src="<?php echo $channel['logo']; ?>" style="height:125px;" loading="lazy" class="h-28 mt-5 m-auto opacity-100">
                                <div class="mb-5 opacity-100">
                                    <h2 class="text-center text-sm font-bold"><?php echo $channel['name']; ?></h2>
                                    <div class="flex justify-center space-x-1">
                                        <p class="text-xs text-center"><?php echo $channel['categoryName']; ?> </p>
                                    </div>
                                </div>
                            </div>
                        </a>
